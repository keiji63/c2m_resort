...
<table id="headerTable" class="table table-hover">
	...
	<tbody>
		...
		<td>{{$index+1}}</td>
			<td>{{x.name}}</td>
<td>
<span ng-if="x.user_type=='1'">พนักงานขายสินค้า</span>
<span ng-if="x.user_type=='2'">พนักงานสต๊อกร้าน</span>
<span ng-if="x.user_type=='4'" style="color: green;">Admin ร้าน</span>
</td>
			<td>{{x.user_email}}</td>
...
<div class="form-group">
	<select id="" class="form-control" ng-model="user_type" style="height: 50px;font-size: 20px;">
		<option value="1">พนักงานขายสินค้า</option>
		<option value="2">พนักงานสต๊อกร้าน</option>
		<option value="4">Admin ร้าน</option>
	</select>
</div>
